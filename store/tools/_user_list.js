...
module.exports.controller_method = async function (unit_step, page_current, option) {
	return new Promise(async (resolve, reject) => {
		...
		const resData = await usersDB.getList(unit_step, page_current, option);
		if (resData.list_data.length) {
			resData.list_data = resData.list_data.map(item=>{
				const data = item.get();
				data.role = data.role.get();
				data.role.verification_route = JSON.parse(data.role.verification_route);
				data.role.verification_api = JSON.parse(data.role.verification_api);
				return data;
			});
		}
